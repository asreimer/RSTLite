...
int RawSetSCF(struct RawData *ptr,int nrang,int mplgs,float *scfd,int snum,int *slist) {
  ...
  for (n=0;n<2;n++) {
    if (ptr->scfd[n]==NULL) tmp=malloc(sizeof(float)*nrang*mplgs);
    else tmp=realloc(ptr->scfd[n],sizeof(float)*nrang*mplgs);
    if (tmp==NULL) return -1;
    ptr->scfd[n]=tmp;
    if (slist==NULL) {
      for (x=0;x<nrang;x++) {
        for (y=0;y<mplgs;y++) {
          tmp[x*mplgs+y]=scfd[2*(x*mplgs+y)+n];
        }
      }
    } else {
      memset(tmp,0,sizeof(float)*nrang*mplgs);
      for (s=0;s<snum;s++) {
        x=slist[s];
        for (y=0;y<mplgs;y++) {
          tmp[x*mplgs+y]=scfd[2*(s*mplgs+y)+n];
        }
      }
    }
  }
